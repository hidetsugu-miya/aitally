...
RSpec.describe(ClaudeCollector) do
  describe '.run' do
    ...
    it 'establishes database connection and starts watcher', :aggregate_failures do
    ...
    it 'registers signal traps and calls sleep', :aggregate_failures do
      allow(Signal).to(receive(:trap))
      allow(described_class).to(receive(:sleep).and_raise(Interrupt))
    ...
    it 'shutdown proc stops watcher, clears connections, and exits', :aggregate_failures do
      shutdown_proc = nil
  ...
  describe '.run integration' do
    ...
    it 'parses fixture JSON and saves sessions to database', :aggregate_failures do
      expect { described_class.run }.to(raise_error(Interrupt))
    ...
    it 'skips projects without session_id', :aggregate_failures do
      expect { described_class.run }.to(raise_error(Interrupt))
